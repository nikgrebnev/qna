...
RSpec.describe Services::FindForOauth do
  ...
  context 'user already has authorisation' do
    it 'returns the user' do
      user.authorizations.create(provider: 'facebook', uid: '123123123123')
  ...
  context  'user has not authorization' do
    context 'user already exists' do
      let(:auth) { OmniAuth::AuthHash.new(provider: 'facebook', uid: '123123123123', info: { email: user.email }) }

      it 'does not create new user' do
        expect { subject.call }.to_not change(User, :count)
      end

      it 'creates authirization for user' do
  ...
  context 'user does not exist' do
    let(:auth) { OmniAuth::AuthHash.new(provider: 'facebook', uid: '123123123123', info: { email: 'new@user.com' }) }
